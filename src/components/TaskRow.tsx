  ...
  return (
    <View style={{ ...ui.card, paddingVertical: 10, paddingHorizontal: 12, flexDirection: "row", alignItems: "center", gap: 10 }}>
      ...
      <View style={{ flex: 1, gap: 2 }}>
        <Text style={{ color: colors.text, fontWeight: "900", textDecorationLine: task.done ? "line-through" : "none", opacity: task.done ? 0.65 : 1 }} numberOfLines={2}>
          {task.title}
        </Text>
        <Text style={{ color: colors.muted, fontSize: 12 }}>Dodano: {formatDateTime(task.createdAt)}</Text>
        {task.done && task.completedAt ? (
          <Text style={{ color: colors.muted, fontSize: 12 }}>Wykonano: {formatDateTime(task.completedAt)}</Text>
        ) : null}
      </View>
      ...
      <View style={{ flexDirection: "row", gap: 8 }}>
        <Pressable
          ...
          style={{
            ...
            backgroundColor: task.done ? "rgba(66,211,146,0.18)" : "rgba(255,255,255,0.04)",
          }}
        >
